...
2. **`_retry_yt_dlp()`** wraps both `get_youtube_info()` and `download_youtube_video()`: 5 attempts, linear backoff (5s, 10s, 15s, 20s). Helps with brief blips; does not reliably beat a sustained block.
...
4. **Video cache** (`VIDEO_CACHE_DIR`, `get_cached_video_path()`, `_enforce_video_cache_limits()`): `download_youtube_video(url, session_id=...)` saves the file keyed by session_id instead of a throwaway temp name. A resumed job (retry, or the queue's stale-job recovery) that only needs to redo `render` reuses this instead of hitting YouTube's bot-check a second time for the same session — this is what actually caused a real incident (2026-09-02): a job that got past `transcribe`/`identifyClips`/`generateCaptions` failed at `render` purely because rendering needed a *second*, independent download attempt that got blocked. Bounded by `VIDEO_CACHE_MAX_AGE_MINUTES` (60) and a hard `VIDEO_CACHE_MAX_TOTAL_MB` cap (3000, oldest-evicted-first) enforced on every write — so many concurrent/failed sessions can't fill the single instance's disk. Not deleted on any stage failure (only on a session's full success) so it survives across retries; only ever deleted early by the cap/TTL sweep.

None of this is a guaranteed fix for the underlying block — that would require either a residential/rotating proxy (cost + infra, no account risk) or cookies from a dedicated YouTube account (free, but account-ban risk and needs periodic manual refresh as cookies expire). Both were discussed with the user (2026-09-02) and deliberately not implemented yet — revisit if failures are still frequent after the mitigations above.
